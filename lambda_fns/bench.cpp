...
static void BM_08_LargeObjLambdaRet(benchmark::State& state);
static void BM_09_LargeObjLambdaRetCall(benchmark::State& state);
...
static void BM_18_SmallObjLambdaRet(benchmark::State& state);
static void BM_19_SmallObjLambdaRetCall(benchmark::State& state);
// GEN_PROTO_END
...
static void BM_18_SmallObjLambdaRet(benchmark::State& state) {
    for (auto _ : state) {
        small_obj_lambda_ret();
    }
}
static void BM_19_SmallObjLambdaRetCall(benchmark::State& state) {
    for (auto _ : state) {
        auto lambda = small_obj_lambda_ret();
        lambda();
    }
}
...
static void BM_09_LargeObjLambdaRetCall(benchmark::State& state) {
    for (auto _ : state) {
        auto lambda = large_obj_lambda_ret();
        lambda();
    }
}
...
BENCHMARK(BM_08_LargeObjLambdaRet);
BENCHMARK(BM_09_LargeObjLambdaRetCall);
...
BENCHMARK(BM_18_SmallObjLambdaRet);
BENCHMARK(BM_19_SmallObjLambdaRetCall);
// GEN_BENCHMARK_END
